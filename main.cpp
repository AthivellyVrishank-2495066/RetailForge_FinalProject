...
std::string centerText(const std::string& text, int width = SCREEN_WIDTH) {
...
std::string centerPrompt(const std::string& text, int width = SCREEN_WIDTH) {
    int totalPadding = width - static_cast<int>(text.length());
    if (totalPadding <= 0) {
        return text;
    }
...
const int DEFAULT_SCREEN_WIDTH = 70;
const int DEFAULT_SCREEN_HEIGHT = 24;

bool ansiEnabled = false;
const char* ANSI_CYAN = "\033[36m";
const char* ANSI_WHITE = "\033[97m";
const char* ANSI_GRAY = "\033[90m";
...
std::string colorText(const std::string& text, const char* color) {
    if (!ansiEnabled || color == NULL || color[0] == '\0') {
        return text;
    }
    return std::string(color) + text + ANSI_RESET;
}

void displayMenuBox(const std::vector<std::pair<std::string, const char*> >& content,
                    const std::string& title,
                    const std::string& subtitle = "") {
    int termWidth, termHeight;
    getTerminalSize(termWidth, termHeight);

    int boxWidth = std::min(termWidth - 10, SCREEN_WIDTH);
    boxWidth = std::max(boxWidth, 40);

    std::vector<std::pair<std::string, const char*> > lines;
    lines.push_back(std::pair<std::string, const char*>(title, ANSI_CYAN));
    if (!subtitle.empty()) {
        lines.push_back(std::pair<std::string, const char*>(subtitle, ANSI_CYAN));
    }
    lines.push_back(std::pair<std::string, const char*>("", NULL));

    for (std::vector<std::pair<std::string, const char*> >::const_iterator item = content.begin();
         item != content.end(); ++item) {
        lines.push_back(std::pair<std::string, const char*>(item->first, item->second));
    }

    int totalLines = static_cast<int>(lines.size()) + 2;
    int topPadding = std::max(0, (termHeight - totalLines) / 2);
...
#ifdef _WIN32
        int ignored = std::system("cls");
#else
        int ignored = std::system("clear");
#endif
        (void)ignored;
    }
}
...
void displayMainMenu() {
    ...
    std::vector<std::pair<std::string, const char*> > menuItems;
    menuItems.push_back(std::pair<std::string, const char*>("1. Product Management", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("2. Supplier Management", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("3. Sales & Purchases", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("4. Reports & Analytics", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("5. Forecasting & Alerts", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("6. System Backup/Restore", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("", NULL));
    menuItems.push_back(std::pair<std::string, const char*>("0. Exit", ANSI_WHITE));

    displayMenuBox(menuItems,
                   "RetailForge - Smart Inventory Management System",
                   "Predictive Demand Forecasting Platform");

    int termWidth, termHeight;
    getTerminalSize(termWidth, termHeight);
    std::cout << centerPrompt("Enter your choice: ", termWidth);
}
...
void displayProductMenu() {
    ...
    std::vector<std::pair<std::string, const char*> > menuItems;
    menuItems.push_back(std::pair<std::string, const char*>("1. Add New Product", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("2. View All Products", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("3. Search Product by ID", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("4. Search Product by Name", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("5. Update Product", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("6. Delete Product", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("7. Filter by Category", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("8. Filter by Stock Level", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("", NULL));
    menuItems.push_back(std::pair<std::string, const char*>("0. Back to Main Menu", ANSI_WHITE));

    displayMenuBox(menuItems, "Product Management");
...
void displaySupplierMenu() {
    ...
    std::vector<std::pair<std::string, const char*> > menuItems;
    menuItems.push_back(std::pair<std::string, const char*>("1. Add New Supplier", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("2. View All Suppliers", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("3. Search Supplier by ID", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("4. Update Supplier", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("5. Delete Supplier", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("", NULL));
    menuItems.push_back(std::pair<std::string, const char*>("0. Back to Main Menu", ANSI_WHITE));

    displayMenuBox(menuItems, "Supplier Management");
...
void displaySalesMenu() {
    ...
    std::vector<std::pair<std::string, const char*> > menuItems;
    menuItems.push_back(std::pair<std::string, const char*>("1. Record a Sale", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("2. Record a Purchase", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("3. View Sales History", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("", NULL));
    menuItems.push_back(std::pair<std::string, const char*>("0. Back to Main Menu", ANSI_WHITE));

    displayMenuBox(menuItems, "Sales & Purchases");
...
void displayReportMenu() {
    ...
    std::vector<std::pair<std::string, const char*> > menuItems;
    menuItems.push_back(std::pair<std::string, const char*>("1. Low Stock Items", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("2. Near Expiry Items", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("3. Top 20 Best-Selling Products", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("4. Slow-Moving Products", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("5. Category-wise Profit & Loss", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("6. Supplier Performance Report", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("7. System Dashboard", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("", NULL));
    menuItems.push_back(std::pair<std::string, const char*>("0. Back to Main Menu", ANSI_WHITE));

    displayMenuBox(menuItems, "Reports & Analytics");
...
void displayForecastMenu() {
    ...
    std::vector<std::pair<std::string, const char*> > menuItems;
    menuItems.push_back(std::pair<std::string, const char*>("1. View Pending Alerts", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("2. Calculate Moving Average", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("3. Predict Demand", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("4. Suggest Reorder Quantity", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("5. Generate All Alerts", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("", NULL));
    menuItems.push_back(std::pair<std::string, const char*>("0. Back to Main Menu", ANSI_WHITE));

    displayMenuBox(menuItems, "Forecasting & Alerts");
...
void displayBackupMenu() {
    ...
    std::vector<std::pair<std::string, const char*> > menuItems;
    menuItems.push_back(std::pair<std::string, const char*>("1. Create Backup", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("2. Restore from Backup", ANSI_WHITE));
    menuItems.push_back(std::pair<std::string, const char*>("", NULL));
    menuItems.push_back(std::pair<std::string, const char*>("0. Cancel", ANSI_WHITE));

    displayMenuBox(menuItems, "System Backup / Restore");
