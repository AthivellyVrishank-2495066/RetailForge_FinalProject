...
void InventoryManager::loadAllData() {
    ...
    if (prodFile.is_open()) {
        ...
        while (std::getline(prodFile, line)) {
            if (firstLine) {
                firstLine = false;
                if (isHeaderLine(line, "productID,name,category,supplierID,currentStock,reorderLevel,costPrice,unitPrice,expiryDate,shelfLifeDays,batchNumber,warrantyMonths,totalSold") ||
                    isHeaderLine(line, "productID,name,category,supplierID,currentStock,reorderLevel,unitPrice,expiryDate,shelfLifeDays,batchNumber,warrantyMonths,totalSold")) {
                    continue;
                }
            }
            if (!line.empty()) {
    ...
    if (salesFile.is_open()) {
        ...
        while (std::getline(salesFile, line)) {
            ...
            if (!line.empty()) {
                ...
                Product* prod = findProduct(trans.getProductID());
                if (prod != NULL) {
                    trans.setCostPrice(prod->getCostPrice());
                }
...
void InventoryManager::saveAllData() {
    ...
    if (prodFile.is_open()) {
        prodFile << "productID,name,category,supplierID,currentStock,reorderLevel,costPrice,unitPrice,expiryDate,shelfLifeDays,batchNumber,warrantyMonths,totalSold\n";
        for (std::map<std::string, Product*>::iterator it = products.begin();
             it != products.end(); ++it) {
...
std::map<std::string, double> InventoryManager::getCategoryProfitLoss() const {
    ...
    // Simple calculation: (sale price - cost price) * quantity for each sale transaction
    for (size_t i = 0; i < salesHistory.size(); ++i) {
        if (salesHistory[i].getTransactionType() == "Sale") {
            Product* prod = findProduct(salesHistory[i].getProductID());
            if (prod != NULL) {
                double profit = (salesHistory[i].getSalePrice() - prod->getCostPrice()) * salesHistory[i].getQuantity();
